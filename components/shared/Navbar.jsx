...
const Navbar = () => {
  return (
    <div className="md:px-8 glassss  px-1 md:py-3 py-1 flex md:justify-between items-center !sticky top-0 z-50 border-bo border-gray-200 backdrop-blur-md !bg-transparen !bg-white/80 dark:!bg-background/30">
      ...
      <div className="lg:flex lg:flex-row flex-col gap-4 hidden">
        <Searching />
        {NAVBAR_ITEM.map((item, index) => (
          <Link
            key={index}
            href={item.href}
            className={`flex items-center !font-bold ${item?.title === "Search" ? "hidden" : ""}`}
          >
